...
enum class HumanAuthErrorCode { NETWORK_FAILURE, CHALLENGE_EXPIRED, CHALLENGE_CONSUMED, CREDENTIAL_REJECTED, NONCE_MISMATCH, PROVIDER_UNAVAILABLE, UNEXPECTED_RESPONSE }
...
class AttentionRouterHumanAuthClient(private val baseUrl:String,private val transport:HumanAuthTransport=OkHttpHumanAuthTransport(baseUrl)):HumanAuthClient {
 override fun requestChallenge()=try { val r=transport.post(PATH,"{}");if(r.statusCode==201) challenge(r.body) else ChallengeResult.Failure(error(r.body)) } catch(_:Exception){ChallengeResult.Failure(HumanAuthErrorCode.NETWORK_FAILURE)}
 override fun verify(challengeId:String,idToken:String):VerifyResult {
  if(challengeId.isBlank()||idToken.isBlank()) return VerifyResult.Failure(HumanAuthErrorCode.CREDENTIAL_REJECTED)
  return try { val r=transport.post("$PATH/$challengeId/verify",buildJsonObject{put("id_token",idToken)}.toString());if(r.statusCode==200) validated(r.body) else VerifyResult.Failure(error(r.body)) } catch(_:Exception){VerifyResult.Failure(HumanAuthErrorCode.NETWORK_FAILURE)}
 }
 ...
 private fun error(body:String)=when(obj(body)?.str("code")){"CHALLENGE_EXPIRED"->HumanAuthErrorCode.CHALLENGE_EXPIRED;"CHALLENGE_CONSUMED"->HumanAuthErrorCode.CHALLENGE_CONSUMED;"CREDENTIAL_REJECTED"->HumanAuthErrorCode.CREDENTIAL_REJECTED;"NONCE_MISMATCH"->HumanAuthErrorCode.NONCE_MISMATCH;"PROVIDER_UNAVAILABLE"->HumanAuthErrorCode.PROVIDER_UNAVAILABLE;"NETWORK_FAILURE"->HumanAuthErrorCode.NETWORK_FAILURE;else->HumanAuthErrorCode.UNEXPECTED_RESPONSE}
 private fun obj(body:String)=try{Json.parseToJsonElement(body).jsonObject}catch(_:Exception){null}
