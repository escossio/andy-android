...
class HumanIdentityEngine(private val deviceReady: Boolean) {
    ...
    fun backendValidated(identity: HumanIdentityReference) = set(if (state == HumanIdentityState.ValidatingBackend && identity.opaqueId.isNotBlank()) HumanIdentityState.Validated(identity) else HumanIdentityState.Failure(HumanIdentityFailure.UNEXPECTED_RESPONSE))
